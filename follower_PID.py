...
class Bot:
    ...
    def vel_follow(self):
        ...
        if((imgM == False and imgL == False) and imgR == False):
            vL ,vD = v,v
        elif((imgL == True and imgR == False) and imgM == False): #sensor izquierda activo
            enew = -1
            vL = v - (kp*enew + kd*(enew - eold) + ki*(enew +  eold))
            vD = 0
            eold = enew
        elif((imgM == False and imgL == False) and imgR == True): #sensor derecha activo
            enew = 1
